...
└── bot/
    ...
    │   ├── tasks.py        # /tasks — меню: «Все задачи» (как /stats) и «Задачи на сегодня»
...
| `/tasks` | Меню: «Все задачи» (как /stats) и «Задачи на сегодня» (отметка) |
...
- **Просмотр задач** (`/tasks`): одно сообщение с меню «Все задачи» (отображение
  идентично `/stats`, стрелочная пагинация) и «Задачи на сегодня» (тот же флоу
  отметки, что в дайджестах); «‹ Назад» возвращает к меню.
