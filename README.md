...
└── bot/
    ...
    │   ├── today.py        # Дайджесты + /check + кнопка «Выполнена» в напоминании
...
| `/check` | Список задач на сегодня и отметка галочками (с правкой текста в реальном времени) |
| `/add` | Добавить задачу |
...
командами `/tasks` («Задачи на сегодня») и `/check`.
...
- **Отметка списком** (`/check`): сообщение с двумя блоками (невыполненные —
  обычным текстом, выполненные — зачёркнутым) и кнопкой «Отметить»; в режиме
  галочек смена отметки в реальном времени перемещает задачу между блоками,
  «Сохранить» фиксирует статусы.
